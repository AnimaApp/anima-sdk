...
## Example: Private Website to Code (Early Preview)

There are times when you might want to generate code from a website that is not public, such as an internal dashboard.
In those cases, Anima SDK offers the ability to generate code starting from an MHTML payload, which can be generated by [Anima's Website Importer Chrome extension](https://chromewebstore.google.com/detail/anima-website-importer/paddhneaanoeljlmdepnheehdkaegblo).

To do so, you can pass the `mhtml` parameter to the `generateCodeFromWebsite` method, instead of the `url` parameter.
